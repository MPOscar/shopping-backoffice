...
import {MenuBrands, MenuBrandsItem, OurPartner} from '../../models/layout';
...
export class MenuBrandsComponent implements AfterViewInit, CanDeactivateMixin {
  ...
  getSlider() {
      this.layoutService.getMenu(this.pageId).subscribe(response => {
  ...
  createUpdateSlider(data: any) {
    ...
    this.data = null;
    this.layoutService.putMenu(this.pageId, data).subscribe(response => {
      this.data = response.data;
