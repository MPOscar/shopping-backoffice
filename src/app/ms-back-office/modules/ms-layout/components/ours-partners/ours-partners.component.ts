...
@Component({
  selector: 'ours-partners',
  templateUrl: './ours-partners.component.html',
  styleUrls: ['./ours-partners.component.scss']
})

@Mixin([CanDeactivateMixin])
export class OursPartnersComponent implements AfterViewInit, CanDeactivateMixin {
  ...
  getOurPartners(){
    this.layoutService.getOurPartners(this.pageId).subscribe(response => {
  ...
  submit(data: any) {
    this.createOrUpdateOurPartnerTab(data);
  }
  ...
  createOrUpdateOurPartnerTab(data: any) {
    data.slides.forEach(element => {
      delete element.id;
    });
    if(data.id){           
      this.layoutService.putOurPartners(this.pageId, data, data.id).subscribe(response => {
      ...
      this.layoutService.postOurPartners(this.pageId, data).subscribe(response => {
        this.unsavedChanges = false;
        this.toastr.success(updatedMessageKey);
        this.getOurPartners();
      },
